...
和枚举类型等，而 **_cv_** 则为限定符，即为：_const_ 和 _volatile_ 则限定符，既不属于基本类型也不属于复合类型。  
...
对应的对象或变量；比如引用有时候指的是引用类型，而有时候指的是引用类型的变量，因为所要阐明的理  
念是一致的_，所以不必要严格地拘泥于定义，根据上下文环境理解即可，这也是复合原文的。
