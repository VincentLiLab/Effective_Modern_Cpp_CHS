...
```  
右值的副本通常是移动构造得到的，而左值的副本则通常是拷贝构造得到的。如果你只知道一个对象是另一个对象的副本的话，那么是不能说构造这个副本是如何的成本大的。例如：在不知道是左值还是右值会传递给 _someFunc_ 的情况下，就不能说创建形参 _w_ 是如何的成本大的。你还必须得知道移动和拷贝 _Widget_ 的成本。  
