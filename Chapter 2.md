...
在概念上，_auto_ 是非常简单的，但是实际上它要比看起来隐晦的多。使用 _auto_ 可以少写一些代码，当然也可以避  
免手动声明类型所带来的正确性问题和效率问题。此外，一些 _auto_ 的类型推导结果虽然是完全地遵守规定的算法  
的，但是站在程序员的角度看却是错误的。当处在这种场景时，重要的是知道如何引导 _auto_ 去得到正确的答案，  
因为通常需要避免回退使用手动类型声明。
...
这条高速公路也没有那些使用解引用 _iterator_ 来声明局部变量时遇到的坑：  
```C++
...
另外，因为 _auto_ 使用了类型推导，见 [_Item 2_](./Chapter%201.md#item-2-理解-auto-的类型推导)，它可以表示只被编译器所知道的类型：  
```C++
...
上，_unsigned_ 是 _32bits_ 而 _std::vector<int>::size_type_ 则是  _64bits_。这意味着在 _32-bit_ _Windows_ 工作的代码可能在 _64-bit_  
_Windows_ 下表现不正确，然后当你将程序从 _32bits_ 移植到 _64bits_ 时，谁会想花时间在这种问题上呢？

使用 _auto_ 可以确保你不需要做那些：  
```C++
  auto sz = v.size();         // sz's type is std::vector<int>::size_type
```

还不确定使用 _auto_ 的明智？那就再考虑下面的代码：  
```C++
...
的临时变量都将会被销毁。如果你写了这个循环的话，你可能会被这个行为所惊讶到，因为几乎可以肯定是你想要  
只是将引用 _p_ 绑定到 _m_ 中的每个元素上而已。
...
却写了 _std::pair<std::string, int>_ ，描述了显式指明类型如何可以导致发生你既不想要也没想到的的隐式转换，如  
果你使用 _auto_ 来做为了目标变量的类型的话，那么你就不再需要担心你正在声明的变量的类型和用于初始化它的  
表达式的类型之间的不匹配了。

因此，这些都是首选 _auto_ 而不是显式类型声明的理由。但 _auto_ 不是完美的。每一个 _auto_ 变量的类型都是根据它的初始化表达式而进行推导的，但有一些初始化表达式的类型却既不是所期待的也不是所符合要求的。关于这种场景发生的条件和关于这种场景你应该怎么做是在 [_Item 2_](./Chapter%201.md#item-2-理解-auto-的类型推导) 和 [_Item 6_](./Chapter%201.md#item-6) 中进行讨论的，所以我不会在这里解决。相反地，我将会转移我的注意力到一个不同的问题上来，那就是在使用 _auto_ 来代替传统类型声明时所产生的源代码可读性问题。

首先，深呼吸，放轻松。_auto_ 是一个可选项，而不是必选项。如果在你的专业判断下，通过使用显式类型声明，你的代码将会是更清晰的、更易维护的或者能以某种方式变得更好的话，那么你可以继续使用显式类型声明。但是请记住 _C++_ 在采用编程语言界普遍称之为类型推断的概念上并没有开创新的领域。其他的静态类型过程式语言，比如：_C#_、_D_、_Scala_ 和 _Visual Basic_，都有或多或少相同的特性，更不用说各种静态类型函数式语言了，比如：_ML_、_Haskell_、_OCaml_ 和 _F#_ 等。在某种程度上，这得归因于动态类型语言的成功，比如：_Perl_、_Python_ 和 _Ruby_，它们其中的变量很少会被显式类型。软件开发社区对类型推断拥有丰富经验，并已证明这种技术与创建和维护大型、工业级代码库之间并无矛盾。

使用 _auto_ 减弱了通过快速看一眼源代码就能知道一个对象的类型的能力，一些开发者对这种情况感到焦虑。

一些开发者对于下面的情况而感到焦虑： 使用 "auto" 减弱了通过快速的看一眼源代码就能确定一个对象的类型的能力。
            然而，可以显示对象类型的 "IDE" 常常可以缓和这个问题，即使考虑到了 "Item 4" 中提到的 "IDE" 类型的显示
            问题，然后在很多的场景中，对于对象类型的抽象理解是和可以和了解实际的类型一样有用的。比如，在不知道究竟
            是是何种类型的容器、计数器或者智能指针的情况下， 知道一个对象是容器、计数器还是智能指针就足够了。假设
            合适的选择了一个变量名，那么一些抽象类型信息也就几乎相差无几了。