...
因为数组形参声明是被当作为指针形参声明的，所以按 _by-value_ 形式传递到模板函数的数组是被推导为指针类型的，这就意味着在模板 _f_ 的调用中，类型形参 _T_ 是被推导为 _const char*_ 的。  
```C++
  f(name);                    // name is array, but T deduced as const char*
```

但是现在来一个曲线球。尽管函数不能声明形参为真正的数组，但是可以声明形参为数组引用。所以如果我们更改模板 _f_ 为按 _by-reference_ 形式来接收它的实参，  
```C++
  template<typename T>
